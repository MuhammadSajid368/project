...
const TeacherEvents = () => {
  ...
  return (
    <Fragment >
        <MetaData title={`Upcoming Events -- Student Dashboard`} />
     <div className='flex'>
     <div>
        <Sidebar />
      </div>
      ...
      {loading ? (
        ...
        <div className='flex flex-wrap -mx-2'>
          {events.map((event) => (
            <div key={event.id} className="w-full sm:w-1/2 md:w-1/2  lg:w-1/3 px-2 mb-4  h-auto shadow-sm ">
              <div className="max-w-sm rounded overflow-hidden shadow-lg bg-white h-auto">
                <img
                  className="w-96 h-52"
                  src={event.image.url}
                  alt="Event Image"
                ...
                <div className="px-6 py-4 bg-gray-100  flex justify-between items-center">
                  ...
                  <Link to={`/event-details/${event._id}`} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                    More Info
                  </Link>
                </div>
              </div>
